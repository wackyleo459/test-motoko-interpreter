...
function ImmutableCodeBlock({id, code, language}) {
  ...
  return (
    <Container as="div">
      <pre id={id} class={language} ref={ref}><code>{code}</code></pre>
    </Container>
  );
}
...
export default function StringWrapper(props) {
  if (props.className === "language-motoko" && ExecutionEnvironment.canUseDOM) {
    if (props.hasOwnProperty('no-repl')) {
      return (
        <ImmutableCodeBlock id={props.name} code={props.children} language="language-motoko" />
      )
    }
    ...
    return (
      ...
      <Container as="div">
        <div className={styles.codeBlockContent}>
          <pre id={props.name} ref={editorRef} class="language-motoko"><code>{code}</code></pre>
          <div className={styles.buttonGroup}>
            <RunButton code={code} setOutput={setOutput} setError={setError} config={extractConfig(props)} />
          </div>
        </div>
      </Container>
        {output || error ? (
